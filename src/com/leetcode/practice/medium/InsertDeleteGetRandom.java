...
class RandomizedSet {
    ...
    public boolean insert(int val) {
        if(!randomMap.containsKey(val)){
            randomMap.put(val,indexList.size());
            indexList.add(val);
            return true;
        }
        return false;
    ...
    public int getRandom() {
        return indexList.get(random.nextInt(randomMap.size()));   //null check not reqd
    }
}
...
public class InsertDeleteGetRandom {
    ...
    randomizedSet.remove(2); // Returns false as 2 does not exist in the set.
    randomizedSet.insert(2); // Inserts 2 to the set, returns true. Set now contains [1,2].
        System.out.println("Value is = " + randomizedSet.getRandom());  // getRandom() should return either 1 or 2 randomly.
    randomizedSet.remove(1); // Removes 1 from the set, returns true. Set now contains [2].
    randomizedSet.insert(2); // 2 was already in the set, so return false.
        System.out.println("Value is = " + randomizedSet.getRandom()); // Since 2 is the only number in the set, getRandom() will always return 2.

    System.out.println("Map is = " + randomizedSet.randomMap);
    }
